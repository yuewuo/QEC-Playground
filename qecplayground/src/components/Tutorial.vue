			...
			<el-collapse-transition>
				<div id="tutorial-has-math" v-show="!collapsed" class="collapse-div" :style="{ 'max-height': max_height + 'px' }">
					<div v-show="step == 0"><!-- Basic Quantum Mechanics -->
						...
						<p>This is an <strong>interactive tutorial</strong> that can help you get some preliminary knowledge about quantum error correction techniques, more specifically, error correction based on surface code. You can read through all the materials or simply jump to <strong style="color: red;">red interactive part like below</strong>. At the end of this tutorial you'll be able to manipulate qubit (quantum bit) errors and run error correction algorithms to correct them.</p>
					...
					<div v-show="step == 4"><!-- Error Decoder -->
						...
						<p>Existing works have proposed several decoders for surface code, for example Lookup Table (LUT) decoder \cite{tomita2014low}, Minimum Weight Perfect Matching (MWPM) decoder \cite{dennis2002topological}, Machine Learning (ML) decoder \cite{torlai2017neural}, Tensor Network (TN) decoder \cite{ferris2014tensor} and Union-Find (UF) decoder \cite{delfosse2017almost}. We use MWPM decoder as a baseline, and propose a new sub-optimal surface code decoder called "naive decoder" to demonstrate how our tool can help developers to visualize failed error corrections and further reason about why this would happen. MWPM decoder is the default decoder in the tutorial to teach people how quantum error correction works interactively. \todo{We also propose a new optimization for MWPM decoder by using machine learning to train the weights of MWPM decoder.}</p>
						<el-card shadow="always" :body-style="{ padding: '10px 20px', background: '#FF5151' }">
