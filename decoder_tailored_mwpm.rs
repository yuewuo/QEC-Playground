...
mod tests {
    ...
    fn tailored_mwpm_decoder_code_capacity_inf_bias_d_3() {  // cargo test tailored_mwpm_decoder_code_capacity_inf_bias_d_3 -- --nocapture
        ...
        let bias_eta = 1e200;
        // build simulator
        let mut simulator = Simulator::new(CodeType::RotatedTailoredCode, CodeType::BuiltinCodeInformation{ noisy_measurements, dp: d, dn: d });
    ...
    fn tailored_mwpm_decoder_periodic_code_capacity_d5() {  // cargo test tailored_mwpm_decoder_periodic_code_capacity_d5 -- --nocapture
        ...
        let bias_eta = 1e6;
        // build simulator
        let mut simulator = Simulator::new(CodeType::PeriodicRotatedTailoredCode, CodeType::BuiltinCodeInformation{ noisy_measurements, dp: d+1, dn: d+1 });
        code_builder_sanity_check(&simulator).unwrap();
        // build error model
        let mut error_model = ErrorModel::new(&simulator);
        let px = p / (1. + bias_eta) / 2.;
