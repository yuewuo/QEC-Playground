...
impl CodeType {
    ...
    pub fn get_left(&self, i: usize, j: usize, code_size: &CodeSize) -> (usize, usize) {
        match self {
            ...
            _ => unimplemented!(
                "left position not implemented for this code type, please fill the implementation"
            ),
        }
    }
    ...
    pub fn get_right(&self, i: usize, j: usize, code_size: &CodeSize) -> (usize, usize) {
        match self {
            &CodeType::RotatedTailoredCode | &CodeType::RotatedTailoredCodeBellInit => (i, j + 1),
            &CodeType::PeriodicRotatedTailoredCode => {
                let dp = code_size.di;
                let dn = code_size.dj;
                let (di, dj) = (dp - 1, dn - 1);
    ...
    pub fn get_down(&self, i: usize, j: usize, code_size: &CodeSize) -> (usize, usize) {
        match self {
            &CodeType::RotatedTailoredCode | &CodeType::RotatedTailoredCodeBellInit => (i + 1, j),
            &CodeType::PeriodicRotatedTailoredCode => {
                let dp = code_size.di;
                let dn = code_size.dj;
                let (di, dj) = (dp - 1, dn - 1);
    ...
    match code_type {
        &CodeType::StandardPlanarCode | &CodeType::RotatedPlanarCode => {
            ...
            if is_rotated {
                assert!(
                    di % 2 == 1,
                    "code distance must be odd integer, current: di = {}",
                    di
                );
                assert!(
                    dj % 2 == 1,
                    "code distance must be odd integer, current: dj = {}",
                    dj
                );
            }
            ...
            let is_present = |i: usize, j: usize| -> bool {
                ...
                assert!(
                    ...
                    "a position cannot be both real and virtual"
                );
                is_this_real || is_this_virtual
            };
            for t in 0..height {
                ...
                for i in 0..vertical {
                    ...
                    for j in 0..horizontal {
                        if is_present(i, j) {
                            ...
                            row_j.push(Some(Box::new(
                                SimulatorNode::new(qubit_type, gate_type, gate_peer.clone())
                                    .set_virtual(
                                        is_virtual(i, j),
                                        gate_peer.map_or(false, |peer| is_virtual(peer.i, peer.j)),
                                    ),
                            )));
                        } else {
                            row_j.push(None);
        ...
        | &CodeType::RotatedTailoredCodeBellInit => {
            ...
            let is_rotated = matches!(code_type, CodeType::RotatedTailoredCode { .. })
                || matches!(code_type, CodeType::RotatedTailoredCodeBellInit { .. });
                                ...
                                match t % simulator.measurement_cycles {
                                    1 => {
                                        ...
                                        if is_bell_init_anc(i, j) && is_bell_init_top(i - 1, j) {
                                            gate_type = GateType::CXGateControl;
                                            gate_peer = Some(pos!(t, i - 1, j));
                                        } else if is_bell_init_top(i, j)
                                            && is_bell_init_anc(i + 1, j)
                                        {
                                            gate_type = GateType::CXGateTarget;
                                            gate_peer = Some(pos!(t, i + 1, j));
                                        } else {
                                ...
                                match t % simulator.measurement_cycles {
                                    ...
                                    3 => {
                                        ...
                                        if j % 2 == 1 {
                                            // operate with right
                                            if is_present(i, j + 1) {
                                                gate_type = if qubit_type == QubitType::Data {
                                                    GateType::CYGateTarget
                                                } else {
                                                    GateType::CXGateControl
                                                };
        ...
        &CodeType::PeriodicRotatedTailoredCode => {
            ...
            assert!(
                dp % 2 == 0,
                "code distance must be even integer, current: dp = {}",
                dp
            );
            assert!(
                dn % 2 == 0,
                "code distance must be even integer, current: dn = {}",
                dn
            );
                ...
                for i in 0..vertical {
                    ...
                    for j in 0..horizontal {
                        if is_present(i, j) {
                            ...
                            row_j.push(Some(Box::new(SimulatorNode::new(
                                qubit_type,
                                gate_type,
                                gate_peer.clone(),
                            ))));
    ...
    simulator_iter!(simulator, position, node, {
        ...
        if node.qubit_type == QubitType::Data {
            if node.gate_type.is_initialization() {
                return Err(format!(
                    "data qubit at {} cannot be initialized: gate_type = {:?}",
        ...
        match node.gate_peer.as_ref() {
            Some(peer_position) => {
                if node.gate_type.is_single_qubit_gate() {
                    return Err(format!(
                        "{} has single qubit gate {:?} should not have peer",
                        position, node.gate_type
                    ));
                }
                ...
                match &peer_node.gate_peer {
                    Some(peer_peer_position) => {
                        if peer_peer_position.as_ref() != position {
                            return Err(format!(
                                "{}, as the peer of {}, doesn't have correct peer but {}",
                                peer_position, position, peer_peer_position
                            ));
                        }
                        if peer_node.gate_type.is_single_qubit_gate() {
                            return Err(format!(
                                "{}, as the peer of {}, doesn't have two-qubit gate",
                    ...
                    None => {
                        return Err(format!(
                            "{}, as the peer of {}, doesn't have peer which is invalid",
                            peer_position, position
                        ))
                    }
                }
            ...
            None => {
                if !node.gate_type.is_single_qubit_gate() {
                    return Err(format!(
                        "two qubit gate {:?} should have peer",
                        node.gate_type
                    ));
                }
            }
...
pub fn code_builder_validate_correction(
    ...
    for (position, error) in correction.iter() {
        assert_eq!(
            position.t, top_t,
            "correction pattern must only be at top layer"
        );
...
#[allow(dead_code)]
pub fn code_builder_sanity_check_correction(
    simulator: &mut Simulator,
    correction: &SparseCorrection,
) -> Result<(), Vec<Position>> {
    // apply the correction directly to the top layer
...
mod tests {
        ...
        ($simulator:ident, $errors:expr, $expected_measurements:expr) => {
            ...
            for (position, error) in $errors.iter() {
                ...
                assert_eq!(
                    node.error,
                    ErrorType::I,
                    "do not set the error at a same position twice: {} {}",
                    position,
                    error
                );
                node.error = *error;
    ...
    #[test]
    fn code_builder_standard_planar_code() {
        // cargo test code_builder_standard_planar_code -- --nocapture
        let di = 7;
        let dj = 5;
        ...
        code_builder_sanity_check(&simulator).unwrap();
        {
            ...
            assert_eq!(
                nodes_count,
                layer_count * (each_layer_real_node_count + each_layer_virtual_node_count)
            );
            assert_eq!(
                virtual_nodes_count,
                layer_count * each_layer_virtual_node_count
            );
        }
        {
        ...
        }
        {
            // check gate sequence
            {
                ...
                assert_eq!(
                    node.gate_peer.as_ref().map(|x| (**x).clone()),
                    Some(pos!(2, 2, 1))
                );
        ...
        }
        {
            ...
            assert_measurement!(
                simulator,
                [(pos!(0, 2, 2), Y)],
                [pos!(6, 1, 2), pos!(6, 2, 1), pos!(6, 2, 3), pos!(6, 3, 2)]
            );
            // Z stabilizer measurement error
            assert_measurement!(
                simulator,
                [(pos!(5, 1, 2), X)],
                [pos!(6, 1, 2), pos!(12, 1, 2)]
            );
    ...
    #[test]
    fn code_builder_standard_tailored_code() {
        // cargo test code_builder_standard_tailored_code -- --nocapture
        let di = 7;
        let dj = 5;
        let noisy_measurements = 3;
        let mut simulator = Simulator::new(
    ...
    #[test]
    fn code_builder_periodic_rotated_tailored_code() {
        // cargo test code_builder_periodic_rotated_tailored_code -- --nocapture
        let di = 7;
        let dj = 5;
        let noisy_measurements = 0;
        let mut simulator = Simulator::new(
            CodeType::PeriodicRotatedTailoredCode,
            CodeSize::new(noisy_measurements, di + 1, dj + 1),
        );
        code_builder_sanity_check(&simulator).unwrap();
        {
            // check stabilizer measurements
            ...
            assert_measurement!(
                simulator,
                [(pos!(0, 1, 5), Z)],
                [pos!(6, 0, 5), pos!(6, 1, 4), pos!(6, 1, 6), pos!(6, 2, 5)]
            );
            assert_measurement!(
                simulator,
                [(pos!(0, 1, 5), Y)],
            ...
            assert_measurement!(
                simulator,
                [(pos!(0, 6, 0), Z)],
                [pos!(6, 0, 5), pos!(6, 1, 6), pos!(6, 5, 0), pos!(6, 6, 1)]
            );
            assert_measurement!(
                simulator,
                [(pos!(0, 6, 0), Y)],
            ...
            assert_measurement!(
                simulator,
                [(pos!(0, 7, 1), Z)],
                [pos!(6, 1, 6), pos!(6, 2, 7), pos!(6, 6, 1), pos!(6, 7, 2)]
            );
            assert_measurement!(
                simulator,
                [(pos!(0, 7, 1), Y)],
            ...
            assert_measurement!(
                simulator,
                [(pos!(0, 13, 7), X)],
                [pos!(6, 5, 0), pos!(6, 7, 12)]
            );
            assert_measurement!(
                simulator,
                [(pos!(0, 13, 7), Z)],
                [pos!(6, 0, 5), pos!(6, 5, 0), pos!(6, 7, 12), pos!(6, 12, 7)]
            );
            assert_measurement!(
                simulator,
                [(pos!(0, 13, 7), Y)],
                [pos!(6, 0, 5), pos!(6, 12, 7)]
            );
            // data qubit at periodic boundary
            assert_measurement!(
                simulator,
                [(pos!(0, 8, 12), X)],
                [pos!(6, 1, 4), pos!(6, 7, 12)]
            );
            assert_measurement!(
                simulator,
                [(pos!(0, 8, 12), Z)],
                [pos!(6, 0, 5), pos!(6, 1, 4), pos!(6, 7, 12), pos!(6, 8, 11)]
            );
            assert_measurement!(
                simulator,
                [(pos!(0, 8, 12), Y)],
                [pos!(6, 0, 5), pos!(6, 8, 11)]
            );
        }
    }
    ...
    fn code_builder_visualize_standard_planar_code() {
        ...
        code_builder_sanity_check(&simulator).unwrap();
        let mut visualizer =
            Visualizer::new(Some(visualize_data_folder() + visualize_filename.as_str())).unwrap();
