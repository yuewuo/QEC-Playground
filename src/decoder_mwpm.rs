...
pub mod mwpm_default_configs {
    ...
    pub fn precompute_complete_model_graph() -> bool {
        false
    } // save for erasure noise model and also large code distance
    ...
    pub fn decode_with_erasure(
        ...
        if to_be_matched.len() > 0 {
                ...
                for erasure_edge in erasure_edges.iter() {
                    match erasure_edge {
                        ErasureEdge::Connection(position1, position2) => {
                            ...
                            let edge12 =
                                node1.edges.get_mut(position2).expect("neighbor must exist");
                            ...
                            let edge21 =
                                node2.edges.get_mut(position1).expect("neighbor must exist");
                            assert_eq!(
                                original_weight12, edge21.weight,
                                "model graph edge must be symmetric"
                            );
                            edge21.weight = 0.; // set to 0 because of erasure
                            erasure_graph_modifier.push_modified_edge(
                        ...
                        ErasureEdge::Boundary(position) => {
                            ...
                            boundary.weight = 0.;
                            erasure_graph_modifier.push_modified_edge(
                                ErasureEdge::Boundary(position.clone()),
                                original_weight,
                            );
                        }
                    }
            ...
            for i in 0..m_len {
                ...
                let (edges, boundary) = self
                    .complete_model_graph
                    .get_edges(position, &to_be_matched);
                match boundary {
                    Some(weight) => {
            ...
            let begin = Instant::now();
            let matching =
                blossom_v::safe_minimum_weight_perfect_matching(node_num, weighted_edges);
            ...
            for i in 0..m_len {
                ...
                if j < i {
                    ...
                    let matching_correction =
                        self.complete_model_graph.build_correction_matching(a, b);
                    correction.extend(&matching_correction);
                } else if j >= m_len {
                    // matched with boundary
                ...
                while erasure_graph_modifier.has_modified_edges() {
                    ...
                    match erasure_edge {
                        ErasureEdge::Connection(position1, position2) => {
                            ...
                            assert_eq!(
                                edge12.weight, 0.,
                                "why a non-zero edge needs to be recovered"
                            );
...
mod tests {
    ...
    fn mwpm_decoder_debug_1() {
        // cargo test mwpm_decoder_debug_1 -- --nocapture
        let d = 5;
