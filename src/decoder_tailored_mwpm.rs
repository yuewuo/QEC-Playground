...
pub mod tailored_mwpm_default_configs {
    pub fn naive_residual_decoding() -> bool {
        false
    }
    pub fn disable_residual_decoding() -> bool {
        false
    }
    pub fn original_residual_weighting() -> bool {
        false
    }
    ...
    pub fn decode(&mut self, sparse_measurement: &SparseMeasurement) -> (SparseCorrection, serde_json::Value) {
        ...
        if !to_be_matched.is_empty() {
            ...
            for i in 0..tailored_len {
                ...
                let [positive_edges, negative_edges] = self
                    .tailored_complete_model_graph
                    .get_tailored_matching_edges(position, &tailored_to_be_matched);
                ...
                for &(i, j, weight) in tailored_weighted_edges.iter() {
                    if i >= tailored_len * 2 || j >= tailored_len * 2 {
                        eprintln!(
                            ...
                            tailored_to_be_matched[i % tailored_len],
                            tailored_to_be_matched[j % tailored_len],
                            weight
                        );
            ...
            for (i, &j) in tailored_matching.iter().enumerate() {
                ...
                if base_i < base_j {
                    // no need to union if base_i == base_j; also no need to union base_i > base_j
            ...
            for &root_i in tailored_cluster_roots.iter() {
                ...
                debug_assert!({
                    // sanity check: indeed even number of StabY and even number of StabX for neutral cluster, otherwise all odd
            ...
            let residual_correction = if self.config.disable_residual_decoding {
                ...
                // reproduce the exact residual-decoding in https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.124.130501
                let mut correction = SparseCorrection::new();
                if charged_cluster_count > 0 {
                    ...
                    let get_cluster_positions = |index: usize| -> Vec<Position> {
                        if index < residual_real_cluster_len {
                            // a real cluster
                    ...
                    let residual_matching = blossom_v::safe_minimum_weight_perfect_matching(
                        residual_to_be_matched_cluster_root.len(),
                        residual_weighted_edges,
                    );
                    ...
                    let apply_correction_with_merged_cluster =
                        |mut_self: &mut TailoredMWPMDecoder,
                         ...
                         cluster_2: Vec<Position>| {
                            ...
                            for positions in [stab_x_positions, stab_y_positions].iter() {
                                for i in (0..positions.len()).step_by(2) {
                                    ...
                                    let matching_correction = mut_self
                                        .tailored_complete_model_graph
                                        .build_correction_neutral_matching(position_1, position_2);
                                    correction.extend(&matching_correction);
                                }
                            }
                        };
                        ...
                        if !neutralized_charged_cluster.contains(&i) {
                            ...
                            if j < residual_real_cluster_len {
                                ...
                                if !was_neutral_j {
                                    ...
                                    assert!(
                                        !neutralized_charged_cluster.contains(&j),
                                        "peer cluster should not already been neutralized"
                                    );
...
mod tests {
    ...
    #[test]
    fn tailored_mwpm_decoder_code_capacity_inf_bias_d_3() {
        // cargo test tailored_mwpm_decoder_code_capacity_inf_bias_d_3 -- --nocapture
        let d = 3;
        ...
        if enable_all {
            ...
            simulator.set_error_check(&noise_model, &pos!(0, 3, 1), &Z);
    ...
    #[test]
    fn tailored_mwpm_decoder_code_capacity() {
        // cargo test tailored_mwpm_decoder_code_capacity -- --nocapture
        let d = 5;
        let noisy_measurements = 0; // perfect measurement
        let p = 0.005;
        let bias_eta = 1e6;
        ...
        if enable_all {
            ...
            simulator.set_error_check(&noise_model, &pos!(0, 7, 3), &Z);
        ...
        if enable_all {
            ...
            simulator.clear_all_errors();
            simulator.set_error_check(&noise_model, &pos!(0, 1, 5), &Z);
            simulator.set_error_check(&noise_model, &pos!(0, 2, 6), &Z);
        ...
        if enable_all {
            ...
            simulator.set_error_check(&noise_model, &pos!(0, 6, 6), &Z);
            simulator.set_error_check(&noise_model, &pos!(0, 6, 8), &Z);
            simulator.set_error_check(&noise_model, &pos!(0, 8, 6), &Z);
            simulator.propagate_errors();
            let sparse_measurement = simulator.generate_sparse_measurement();
            assert_eq!(
    ...
    #[test]
    fn tailored_mwpm_decoder_code_capacity_residual() {
        // cargo test tailored_mwpm_decoder_code_capacity_residual -- --nocapture
        let d = 5;
        let noisy_measurements = 0; // perfect measurement
        let p = 0.01;
        let bias_eta = 10.;
        ...
        );
        {
            ...
            simulator.clear_all_errors();
            simulator.set_error_check(&noise_model, &pos!(0, 7, 5), &Y);
            simulator.set_error_check(&noise_model, &pos!(0, 8, 4), &Z);
            simulator.propagate_errors();
            let sparse_measurement = simulator.generate_sparse_measurement();
        ...
        if false {
            // debug 8: residual decoding with charged node
            simulator.clear_all_errors();
            simulator.set_error_check(&noise_model, &pos!(0, 6, 4), &Z);
            simulator.set_error_check(&noise_model, &pos!(0, 5, 5), &Z);
    ...
    #[test]
    fn tailored_mwpm_decoder_code_capacity_residual_2() {
        // cargo test tailored_mwpm_decoder_code_capacity_residual_2 -- --nocapture
        let d = 7;
        let noisy_measurements = 0; // perfect measurement
        let p = 0.05;
        let bias_eta = 10.;
        ...
        );
        {
            ...
            simulator.clear_all_errors();
            simulator.set_error_check(&noise_model, &pos!(0, 6, 4), &Z);
            simulator.set_error_check(&noise_model, &pos!(0, 6, 8), &Y);
            simulator.set_error_check(&noise_model, &pos!(0, 8, 6), &Z);
            simulator.set_error_check(&noise_model, &pos!(0, 10, 8), &Z);
            simulator.set_error_check(&noise_model, &pos!(0, 11, 9), &Y);
            simulator.propagate_errors();
            let sparse_measurement = simulator.generate_sparse_measurement();
            println!("sparse_measurement: {:?}", sparse_measurement);
            let (correction, _runtime_statistics) = tailored_mwpm_decoder.decode(&sparse_measurement);
    ...
    #[test]
    fn tailored_mwpm_decoder_deadlock_1() {
        // cargo test tailored_mwpm_decoder_deadlock_1 -- --nocapture
        let d = 11;
        let noisy_measurements = 0; // perfect measurement
    ...
    #[test]
    fn tailored_mwpm_decoder_periodic_code_capacity_d5() {
        // cargo test tailored_mwpm_decoder_periodic_code_capacity_d5 -- --nocapture
        let d = 5;
        let noisy_measurements = 0; // perfect measurement
        let p = 0.005;
        let bias_eta = 1e6;
        ...
        let mut simulator = Simulator::new(
            ...
            CodeSize::new(noisy_measurements, d + 1, d + 1),
        );
        code_builder_sanity_check(&simulator).unwrap();
        // build noise model
        let mut noise_model = NoiseModel::new(&simulator);
    ...
    #[test]
    fn tailored_mwpm_decoder_periodic_code_capacity_d7() {
        // cargo test tailored_mwpm_decoder_periodic_code_capacity_d7 -- --nocapture
        let d = 7;
        let noisy_measurements = 0; // perfect measurement
        let p = 0.001;
        let bias_eta = 1e200;
    ...
    #[test]
    fn tailored_mwpm_decoder_debug_1() {
        // cargo test tailored_mwpm_decoder_debug_1 -- --nocapture
        let d = 5;
    ...
    #[test]
    fn tailored_mwpm_decoder_debug_2() {
        // cargo test tailored_mwpm_decoder_debug_2 -- --nocapture
    ...
    // {"[5][3][4]":"Y","[5][7][8]":"Y","[5][9][6]":"Y","[6][5][5]":"Z","[6][5][7]":"Z"}
    #[test]
    fn tailored_mwpm_decoder_debug_3() {
        // cargo test tailored_mwpm_decoder_debug_3 -- --nocapture
        let visualizer_filename = "tailored_mwpm_decoder_debug_3".to_string();
        print_visualize_link(visualizer_filename.clone());
        let d = 5;
        ...
        let decoder_config = json!({
            ...
            "log_matchings": true,
            "use_unfixed_stabilizer_edges": true,
        });
        let mut tailored_mwpm_decoder = TailoredMWPMDecoder::new(
            &Arc::new(simulator.clone()),
            Arc::clone(&noise_model),
        ...
        let error_pattern: SparseErrorPattern = serde_json::from_str(r#"{"[6][5][5]":"Z","[6][5][7]":"Z"}"#).unwrap();
        // println!("{:?}", error_pattern);
        simulator.load_sparse_error_pattern(&error_pattern, &noise_model).unwrap();
        simulator.propagate_errors();
        let sparse_measurement = simulator.generate_sparse_measurement();
