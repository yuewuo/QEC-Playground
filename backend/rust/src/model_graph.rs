...
impl ModelGraph {
                ...
                if p > 0. || is_erasure {  // use possible errors to build `all_edges`
                    ...
                    if sparse_measurement_real.len() == 2 {  // normal edge
                        ...
                        if is_same_type && (p > 0. || is_erasure) {
                            ...
                            self.add_edge_between(position2, position1, p, weight_of(p), sparse_errors.clone(), sparse_correction.clone());
                        }
                    }
                }
            }
    ...
    pub fn add_edge_between(&mut self, source: &Position, target: &Position, probability: f64, weight: f64, error_pattern: Arc<SparseErrorPattern>, correction: Arc<SparseCorrection>) {
