    ...
    let matches = clap_app!(QECPlayground =>
        ...
        (@subcommand test => (about: "testing features")
            ...
            (@subcommand offer_decoder_study => (about: "find some error cases in which offer decoder fails but MWPM decoder succeeds")
                ...
                (@arg max_resend: -r --max_resend +takes_value "maximum rounds to resend offer, default to 100")
                (@arg max_cycles: -m --max_cycles +takes_value "maximum cycles to run, corresponding to clock cycle in real hardware, default to 1000")
            )
        )
