    ...
    let matches = clap_app!(QECPlayground =>
        ...
        (@subcommand tool => (about: "tools")
            ...
            (@subcommand fault_tolerant_benchmark => (about: "benchmark fault tolerant algorithm")
                ...
                (@arg fast_benchmark_disable_additional_error: --fast_benchmark_disable_additional_error "do not add additional error when running fast benchmark")
                (@arg fast_benchmark_use_fake_decoder: --fast_benchmark_use_fake_decoder "fake decoder succeed only if mini_sync_time")
                (@arg fast_benchmark_no_weighted_path_sampling: --fast_benchmark_no_weighted_path_sampling "by default is weighted sampling which is expected to converge faster")
                (@arg fast_benchmark_no_weighted_assignment_sampling: --fast_benchmark_no_weighted_assignment_sampling "by default is weighted sampling which is expected to converge faster")
                (@arg fast_benchmark_assignment_sampling_amount: --fast_benchmark_assignment_sampling_amount +takes_value "by default to 10")
            )
