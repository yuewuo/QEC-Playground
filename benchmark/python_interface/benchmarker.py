...
class QubitNoiseModel:
    ...
    def _get_is_virtual(self):
        return self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]["is_virtual"]
    is_virtual = property(
    ...
    def _get_is_peer_virtual(self):
        return self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]["is_peer_virtual"]
    ...
    def _get_type(self):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
    ...
    def _get_gate_type(self):
        return self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]["gate_type"]
    gate = property(
        fget=_get_gate_type,
    ...
    def __repr__(self):
        ...
        rst += f"  {alert if node['noise_model']['pp']['px'] > 0 else ' '}     pX = {node['noise_model']['pp']['px']}\n"
        rst += f"  {alert if node['noise_model']['pp']['pz'] > 0 else ' '}     pZ = {node['noise_model']['pp']['pz']}\n"
        rst += f"  {alert if node['noise_model']['pp']['py'] > 0 else ' '}     pY = {node['noise_model']['pp']['py']}\n"
        rst += f"  {alert if node['noise_model']['pp']['px']+node['noise_model']['pp']['pz']+node['noise_model']['pp']['py'] > 0 else ' '}       => pX+pZ+pY = {node['noise_model']['pp']['px']+node['noise_model']['pp']['pz']+node['noise_model']['pp']['py']}\n"
        rst += f"    Single-Qubit Erasure Error:\n"
        rst += f"  {alert if node['noise_model']['pe'] > 0 else ' '}     pE: {node['noise_model']['pe']}"
        tensor = f"\u00D7"
        if node['gate_peer'] is not None:
            ...
            correlated_noise_model = node['noise_model']['corr_pp']
            rst += f"  {alert if correlated_noise_model['pix'] > 0 else ' '}     pIX = {correlated_noise_model['pix']}\n"
            rst += f"  {alert if correlated_noise_model['piz'] > 0 else ' '}     pIZ = {correlated_noise_model['piz']}\n"
            rst += f"  {alert if correlated_noise_model['piy'] > 0 else ' '}     pIY = {correlated_noise_model['piy']}\n"
            rst += f"  {alert if correlated_noise_model['pxi'] > 0 else ' '}     pXI = {correlated_noise_model['pxi']}\n"
            rst += f"  {alert if correlated_noise_model['pxx'] > 0 else ' '}     pXX = {correlated_noise_model['pxx']}\n"
            rst += f"  {alert if correlated_noise_model['pxz'] > 0 else ' '}     pXZ = {correlated_noise_model['pxz']}\n"
            rst += f"  {alert if correlated_noise_model['pxy'] > 0 else ' '}     pXY = {correlated_noise_model['pxy']}\n"
            rst += f"  {alert if correlated_noise_model['pzi'] > 0 else ' '}     pZI = {correlated_noise_model['pzi']}\n"
            rst += f"  {alert if correlated_noise_model['pzx'] > 0 else ' '}     pZX = {correlated_noise_model['pzx']}\n"
            rst += f"  {alert if correlated_noise_model['pzz'] > 0 else ' '}     pZZ = {correlated_noise_model['pzz']}\n"
            rst += f"  {alert if correlated_noise_model['pzy'] > 0 else ' '}     pZY = {correlated_noise_model['pzy']}\n"
            rst += f"  {alert if correlated_noise_model['pyi'] > 0 else ' '}     pYI = {correlated_noise_model['pyi']}\n"
            rst += f"  {alert if correlated_noise_model['pyx'] > 0 else ' '}     pYX = {correlated_noise_model['pyx']}\n"
            rst += f"  {alert if correlated_noise_model['pyz'] > 0 else ' '}     pYZ = {correlated_noise_model['pyz']}\n"
            rst += f"  {alert if correlated_noise_model['pyy'] > 0 else ' '}     pYY = {correlated_noise_model['pyy']}\n"
            correlated_sum = 0
            for name in correlated_noise_model:
                correlated_sum += correlated_noise_model[name]
            rst += f"  {alert if correlated_sum > 0 else ' '}       => sum = {correlated_sum}\n"
            rst += f"      Correlated Two-Qubit Erasure Error: (this {tensor} peer)\n"
            correlated_erasure_noise_model = node['noise_model']['corr_pe']
            rst += f"  {alert if correlated_erasure_noise_model['pie'] > 0 else ' '}     pIE = {correlated_erasure_noise_model['pie']}\n"
            rst += f"  {alert if correlated_erasure_noise_model['pei'] > 0 else ' '}     pEI = {correlated_erasure_noise_model['pei']}\n"
            rst += f"  {alert if correlated_erasure_noise_model['pee'] > 0 else ' '}     pEE = {correlated_erasure_noise_model['pee']}\n"
            correlated_erasure_sum = 0.
            for name in correlated_erasure_noise_model:
                correlated_erasure_sum += correlated_erasure_noise_model[name]
            rst += f"  {alert if correlated_erasure_sum > 0 else ' '}       => sum = {correlated_erasure_sum}"
        return rst
    def _get_pX(self):
        return self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]['noise_model']['pp']['px']
    def _set_pX(self, error_rate):
        self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]['noise_model']['pp']['px'] = error_rate
    pX = property(
        fset=_set_pX,
        fget=_get_pX,
    ...
    def _get_pZ(self):
        return self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]['noise_model']['pp']['pz']
    def _set_pZ(self, error_rate):
        self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]['noise_model']['pp']['pz'] = error_rate
    pZ = property(
        fset=_set_pZ,
        fget=_get_pZ,
    ...
    def _get_pY(self):
        return self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]['noise_model']['pp']['py']
    def _set_pY(self, error_rate):
        self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]['noise_model']['pp']['py'] = error_rate
    pY = property(
        fset=_set_pY,
        fget=_get_pY,
    ...
    def _get_pE(self):
        return self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]['noise_model']['erasure_error_rate']
    def _set_pE(self, error_rate):
        self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]['noise_model']['erasure_error_rate'] = error_rate
    pE = property(
        fset=_set_pE,
        fget=_get_pE,
    ...
    def _get_pIX(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_ix']
    def _set_pIX(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of IX is invalid")
        node['noise_model']['corr_pp']['error_rate_ix'] = error_rate
    pIX = property(
        fset=_set_pIX,
        fget=_get_pIX,
    ...
    def _get_pIZ(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_iz']
    def _set_pIZ(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of IZ is invalid")
        node['noise_model']['corr_pp']['error_rate_iz'] = error_rate
    pIZ = property(
        fset=_set_pIZ,
        fget=_get_pIZ,
    ...
    def _get_pIY(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_iy']
    def _set_pIY(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of IY is invalid")
        node['noise_model']['corr_pp']['error_rate_iy'] = error_rate
    pIY = property(
        fset=_set_pIY,
        fget=_get_pIY,
    ...
    def _get_pXI(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_xi']
    def _set_pXI(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of XI is invalid")
        node['noise_model']['corr_pp']['error_rate_xi'] = error_rate
    pXI = property(
        fset=_set_pXI,
        fget=_get_pXI,
    ...
    def _get_pXX(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_xx']
    def _set_pXX(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of XX is invalid")
        node['noise_model']['corr_pp']['error_rate_xx'] = error_rate
    pXX = property(
        fset=_set_pXX,
        fget=_get_pXX,
    ...
    def _get_pXZ(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_xz']
    def _set_pXZ(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of XZ is invalid")
        node['noise_model']['corr_pp']['error_rate_xz'] = error_rate
    pXZ = property(
        fset=_set_pXZ,
        fget=_get_pXZ,
    ...
    def _get_pXY(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_xy']
    def _set_pXY(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of XY is invalid")
        node['noise_model']['corr_pp']['error_rate_xy'] = error_rate
    pXY = property(
        fset=_set_pXY,
        fget=_get_pXY,
    ...
    def _get_pZI(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_zi']
    def _set_pZI(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of ZI is invalid")
        node['noise_model']['corr_pp']['error_rate_zi'] = error_rate
    pZI = property(
        fset=_set_pZI,
        fget=_get_pZI,
    ...
    def _get_pZX(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_zx']
    def _set_pZX(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of ZX is invalid")
        node['noise_model']['corr_pp']['error_rate_zx'] = error_rate
    pZX = property(
        fset=_set_pZX,
        fget=_get_pZX,
    ...
    def _get_pZZ(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_zz']
    def _set_pZZ(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of ZZ is invalid")
        node['noise_model']['corr_pp']['error_rate_zz'] = error_rate
    pZZ = property(
        fset=_set_pZZ,
        fget=_get_pZZ,
    ...
    def _get_pZY(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_zy']
    def _set_pZY(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of ZY is invalid")
        node['noise_model']['corr_pp']['error_rate_zy'] = error_rate
    pZY = property(
        fset=_set_pZY,
        fget=_get_pZY,
    ...
    def _get_pYI(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_yi']
    def _set_pYI(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of YI is invalid")
        node['noise_model']['corr_pp']['error_rate_yi'] = error_rate
    pYI = property(
        fset=_set_pYI,
        fget=_get_pYI,
    ...
    def _get_pYX(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_yx']
    def _set_pYX(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of YX is invalid")
        node['noise_model']['corr_pp']['error_rate_yx'] = error_rate
    pYX = property(
        fset=_set_pYX,
        fget=_get_pYX,
    ...
    def _get_pYZ(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_yz']
    def _set_pYZ(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of YZ is invalid")
        node['noise_model']['corr_pp']['error_rate_yz'] = error_rate
    pYZ = property(
        fset=_set_pYZ,
        fget=_get_pYZ,
    ...
    def _get_pYY(self):
        ...
        return node['noise_model']['corr_pp']['error_rate_yy']
    def _set_pYY(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of YY is invalid")
        node['noise_model']['corr_pp']['error_rate_yy'] = error_rate
    pYY = property(
        fset=_set_pYY,
        fget=_get_pYY,
    ...
    def _get_pIE(self):
        ...
        return node['noise_model']['corr_pe']['error_rate_ie']
    def _set_pIE(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of IE is invalid")
        node['noise_model']['corr_pe']['error_rate_ie'] = error_rate
    pIE = property(
        fset=_set_pIE,
        fget=_get_pIE,
    ...
    def _get_pEI(self):
        ...
        return node['noise_model']['corr_pe']['error_rate_ei']
    def _set_pEI(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of EI is invalid")
        node['noise_model']['corr_pe']['error_rate_ei'] = error_rate
    pEI = property(
        fset=_set_pEI,
        fget=_get_pEI,
    ...
    def _get_pEE(self):
        ...
        return node['noise_model']['corr_pe']['error_rate_ee']
    def _set_pEE(self, error_rate):
        node = self.source.noise_model["nodes"][self.position.t][self.position.i][self.position.j]
        if node["gate_peer"] is None:
            raise Exception(f"qubit[{self.position.t}][{self.position.i}][{self.position.j}] doesn't have two-qubit gate here, error rate of EE is invalid")
        node['noise_model']['corr_pe']['error_rate_ee'] = error_rate
    pEE = property(
        fset=_set_pEE,
        fget=_get_pEE,
...
class NoiseModelName:
    def __init__(self, noise_model, di, dj, measurement_rounds, p, pe, configuration):
        ...
        if position.t < len(nodes) and nodes[position.t] is not None:
            ...
            if position.i < len(nodes_row_0) and nodes_row_0[position.i] is not None:
                ...
                if position.j < len(nodes_row_1) and nodes_row_1[position.j] is not None:
                    # node = nodes_row_1[j]
                    return QubitNoiseModel(self, position)
        # raise Exception(f"position [{t}][{i}][{j}] not exist")
        return None

    def _get_height(self):
        return self.noise_model["height"]
    height = property(
        fget=_get_height,
        doc="0 <= t <= height"
    )
    ...
    def _get_vertical(self):
        return self.noise_model["vertical"]
    vertical = property(
        fget=_get_vertical,
        doc="0 <= i <= vertical"
    )
    ...
    def _get_horizontal(self):
        return self.noise_model["horizontal"]
    horizontal = property(
        fget=_get_horizontal,
        doc="0 <= j <= horizontal"
    )

    def visualize(self, api_base_url="https://qec.wuyue98.cn/api", viewer_url="https://qec.wuyue98.cn/NoiseModelViewer2D.html"):
        ...
        modified_noise_model = json.dumps(self.noise_model)
        data = json.dumps({"value": modified_noise_model}).encode('utf-8')
        req = request.Request(api_base_url + "/new_temporary_store", method="POST", data=data)
        req.add_header('Content-Type', 'application/json')
        try:
            response = request.urlopen(req)
            noise_model_temporary_id = response.read().decode('utf-8')
        ...
        query_paramters = f"?p={float(self.p)}&pe={float(self.pe)}&parameters={configuration_escaped}&noise_model_temporary_id={noise_model_temporary_id}"
        if viewer_url is not None:
            url = viewer_url + query_paramters
            webbrowser.open(url, new=2)
        else:
            print(f"viewer_url not provided, please open html viewer (e.g. QEC-Playground/frontend/NoiseModelViewer2D.html) and append the following parameters to it:")
            print(f"    {query_paramters}")
        download_url = api_base_url + "/get_temporary_store/" + noise_model_temporary_id
        print(f"[info] you can download the noise model Json file using: {download_url}")
        return noise_model_temporary_id

    def save(self, filepath):
        with open(filepath, "w", encoding="utf8") as f:
            modified_noise_model = json.dumps(self.noise_model)
            f.write(modified_noise_model)
            f.flush()
    ...
    def run_benchmark(self, max_N=100000, min_error_cases=3000, time_budget=None, verbose=False):
        ...
        with open(out_filename, "w", encoding="utf8") as f:
            ...
            f.write(modified_noise_model)
            f.flush()
            command = qec_playground_benchmark_simulator_runner_vec_command([self.p], [self.di], [self.dj], [self.measurement_rounds], ["--pes", f"[{self.pe}]"] + self.configuration + ["--load_noise_model_from_file"
...
def fetch_noise_model(di, dj, noisy_measurement_rounds, p=0, pe=0, configuration=[]):
    ...
    # print(json_string)
    return NoiseModelName(json.loads(json_string), di, dj, noisy_measurement_rounds, p, pe, configuration)
