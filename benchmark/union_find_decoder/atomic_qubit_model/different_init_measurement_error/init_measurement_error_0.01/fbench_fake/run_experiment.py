...
p_vec = [0.5 * (10 ** (- i / 5)) for i in range(5 * 32 + 1)]
print(p_vec)
...
UF_parameters = f"-p{STO(0)} --time_budget {600} --use_xzzx_code --error_model OnlyGateErrorCircuitLevelCorrelatedErasure --use_fast_benchmark --fbench_use_fake_decoder --fbench_disable_additional_error --fbench_target_dev 1e-2".split(" ") + ["--error_model_configuration", error_model_configuration]  # a maximum 20min for each point
