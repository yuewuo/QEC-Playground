...
with open("decoding_time_DUF.txt", "r", encoding="utf-8") as f:
    lines = f.readlines()
    for line in lines:
        line = line.strip(" \r\n")
...
for i in range(0, len(configurations)):
    ...
    for e in vec:
        if e["error"]:
            ...
            duf_clock_cycles_vec.append(e["duf_clock_cycles"])
    ...
    print(f"duf_clock_cycles: {average(duf_clock_cycles_vec)}, {sorted(duf_clock_cycles_vec)[upper_idx]}")
    if config["di"] >= 4:
        fitting_data.append((config["di"], average(duf_clock_cycles_vec)))
