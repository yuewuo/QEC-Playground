...
pairs = [ (di, di, 0) for di in [5, 7, 9, 11, 13, 15] ]  # (di, dj, T)
p_vec = [0.5 * (10 ** (- i / 3)) for i in range(9)]
...
UF_parameters = f"-b10 -p{num_threads} --shallow_error_on_bottom --use_xzzx_code --bias_eta 100 --decoder UF --max_half_weight 10".split(" ")
