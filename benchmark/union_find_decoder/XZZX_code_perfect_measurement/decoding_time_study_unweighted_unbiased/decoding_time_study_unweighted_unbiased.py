...
UF_parameters = f"-b100 -p{num_threads} --shallow_error_on_bottom --use_xzzx_code --bias_eta 1 --decoder UF --max_half_weight 1 --detailed_runtime_statistics".split(" ")
UF_command = qec_playground_fault_tolerant_MWPM_simulator_runner_vec_command(p_vec, di_vec, dj_vec, T_vec, UF_parameters + ["--log_runtime_statistics", "target/pm_decoding_time_study_unweighted_unbiased.txt"], max_N=max_N, min_error_cases=min_error_cases)
