...
for p in p_vec:
    ...
    UF_parameters = f"-p{num_threads} --code_type StandardXZZXCode --noise_model generic-biased-with-biased-cx --bias_eta 100 --decoder union-find --decoder_config {{\"max_half_weight\":{max_half_weight},\"benchmark_skip_building_correction\":true,\"use_combined_probability\":false}} --parallel_init {parallel_init} --use_brief_edge".split(" ")
    # UF_parameters = f"-p{num_threads} --code_type StandardXZZXCode --noise_model phenomenological --bias_eta 100 --decoder union-find --decoder_config {{\"max_half_weight\":{max_half_weight},\"benchmark_skip_building_correction\":true}}".split(" ")
    # UF_parameters = f"-p{num_threads} --code_type StandardXZZXCode --noise_model phenomenological --decoder union-find --decoder_config {{\"max_half_weight\":{max_half_weight},\"benchmark_skip_building_correction\":true}}".split(" ")
    # UF_parameters = f"-p{num_threads} --noise_model phenomenological --bias_eta +inf --decoder union-find --decoder_config {{\"max_half_weight\":{max_half_weight},\"benchmark_skip_building_correction\":true}}".split(" ")
