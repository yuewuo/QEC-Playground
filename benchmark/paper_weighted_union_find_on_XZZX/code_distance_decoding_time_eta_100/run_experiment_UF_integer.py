...
for p in p_vec:
    ...
    UF_parameters = f"-p{num_threads} --code_type StandardXZZXCode --error_model generic-biased-with-biased-cx --bias_eta 100 --decoder union-find --decoder_config {{\"max_half_weight\":10}}".split(" ")
    # UF_parameters = f"-p{num_threads} --code_type StandardXZZXCode --error_model generic-biased-with-biased-cx --bias_eta 100 --decoder union-find --decoder_config {{\"use_real_weighted\":true,\"max_half_weight\":1000000}}".split(" ")
