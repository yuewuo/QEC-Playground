...
T = 100
pairs = [ (d, 3*d, T) for d in [3, 4, 5, 6, 8, 10, 12, 16, 20, 24] ]  # (di, dj, T)
...
# this time can be substracted from overall time, because it's always O(N) and since p can be very small, including this part may shallow other parts
...
for p in p_vec:
    ...
    # UF_parameters = f"-p{num_threads} --code_type StandardXZZXCode --error_model generic-biased-with-biased-cx --bias_eta 100 --decoder union-find --decoder_config {{\"max_half_weight\":3}}".split(" ")
    UF_parameters = f"-p{num_threads} --code_type StandardXZZXCode --error_model generic-biased-with-biased-cx --bias_eta 100 --decoder union-find --decoder_config {{\"use_real_weighted\":true}}".split(" ")
