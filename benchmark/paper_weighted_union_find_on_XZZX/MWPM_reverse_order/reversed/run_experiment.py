...
di_vec = [3, 5, 7, 9]
p_vec = [0.09, 0.091, 0.092, 0.093, 0.094, 0.095, 0.096, 0.097, 0.098, 0.099, 0.1]
min_error_cases = 4000000
# min_error_cases = 10  # debug

max_N = 0

if slurm_distribute.SLURM_DISTRIBUTE_ENABLED:
    input("[WARNING] you need to compile the program with `cargo build --release --features MWPM_reverse_order` manually in SLURM [press any key to continue]")
...
parameters = f"-p{STO(0)} --decoder mwpm --decoder_config {{\"pcmg\":true}} --time_budget {3600*2}".split(" ")

compile_code_if_necessary(["--features", "MWPM_reverse_order"])
...
if slurm_distribute.SLURM_DISTRIBUTE_ENABLED:
    input("[WARNING] you need to recompile the program `cargo build --release` manually to reset the binary to default one [press any key to continue]")
