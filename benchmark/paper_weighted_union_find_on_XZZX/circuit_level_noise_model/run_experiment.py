...
def experiment(slurm_commands_vec = None, run_command_get_stdout=run_qec_playground_command_get_stdout):
    ...
    for (name, noise_model) in [("biased", "GenericBiasedWithBiasedCX"), ("standard", "GenericBiasedWithStandardCX")]:
        for di in di_vec:
            filename = os.path.join(os.path.dirname(__file__), f"{name}_{di}.txt")
            
            results = []
            for p in p_vec:
                command = qec_playground_fault_tolerant_MWPM_simulator_runner_vec_command([p], [di], [3 * di], [3 * di], parameters + ["--noise_model", f"{noise_model}"], max_N=max_N, min_error_cases=min_error_cases)
