...
for name in ["pointer", "index"]:
    ...
    dist_filename = os.path.join(this_dir, f"{name}_{d}.distribution")
    with open(dist_filename, "r", encoding="utf8") as f:
        distribution = TimeDistribution.from_line(f.read())

    pL_filename = os.path.join(this_dir, f"result_{name}.txt")
    ...
    print(
        f"{name}: average decoding time: {distribution.average_latency():.3e}s, pL = {pL:.3e} (confidence = {confidence:.2e})"
    )
