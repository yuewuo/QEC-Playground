...
for config in configurations:
    ...
    for suffix in suffixes:
        ...
        print(
            f"{config.name}{suffix}: average decoding time: {distribution.average_latency():.3e}s, pL = {pL:.3e} (confidence = {confidence:.2e})"
        )
        ...
        plt.savefig(f"{config.name}_{d}{suffix}.pdf")
