...
Since this fix will be permanent and not provided in command line interface, 
it can only be disabled using build parameters!!!!

If running on a cluster, you should build it with "cargo build --release --no-default-features"

If some other default features exist, you must also manually enable them to compare fairly, using --features ["f1", "f2", ...]
...
di_vec = [3,5,7,9,11]  # rotated surface code only supports odd number code distances
...
parameters = f"-p{STO(0)} --time_budget {3600} --rotated_planar_code".split(" ")

compile_code_if_necessary(["--no-default-features"])
...
def experiment(slurm_commands_vec = None, run_command_get_stdout=run_qec_playground_command_get_stdout):
    ...
    for di in di_vec:
        filename = os.path.join(os.path.dirname(__file__), f"before_fix_{di}.txt")

        results = []
        for p in p_vec:
            command = qec_playground_fault_tolerant_MWPM_simulator_runner_vec_command([p], [di], [di], [di], parameters, max_N=max_N, min_error_cases=min_error_cases)
