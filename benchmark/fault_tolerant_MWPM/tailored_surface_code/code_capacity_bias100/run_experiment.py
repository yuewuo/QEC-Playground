...
parameters = f"-p{STO(0)} --time_budget {3600} --use_rotated_tailored_code --shallow_error_on_bottom --bias_eta 100".split(" ")
...
def experiment(slurm_commands_vec = None, run_command_get_stdout=run_qec_playground_command_get_stdout):
    ...
    for di in di_vec:
        ...
        results = []
        for p in p_vec:
            command = qec_playground_fault_tolerant_MWPM_simulator_runner_vec_command([p], [di], [di], [0], parameters, max_N=max_N, min_error_cases=min_error_cases)
