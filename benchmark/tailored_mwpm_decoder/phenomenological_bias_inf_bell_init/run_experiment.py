...
parameters = f"-p{STO(0)} --time_budget {3600} --code_type RotatedTailoredCode --bias_eta 1e200 --decoder tailored-mwpm --decoder_config {{\"pcmg\":true}} --noise_model tailored-sc-bell-init-phenomenological".split(" ")
...
def experiment(slurm_commands_vec = None, run_command_get_stdout=run_qec_playground_command_get_stdout):
    ...
    for di in di_vec:
        ...
        results = []
        for p in p_vec:
            command = qec_playground_benchmark_simulator_runner_vec_command([p], [di], [di], [di+1], parameters, max_N=max_N, min_error_cases=min_error_cases)
